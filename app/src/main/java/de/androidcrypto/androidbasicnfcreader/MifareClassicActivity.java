...
public class MifareClassicActivity extends BaseNfcActivity implements View.OnClickListener {
    ...
    protected void onNewIntent(Intent intent) {
        ...
        readGuardCard(tag);
//        String id = NFCUtil.byte2HexString(MifareClassic.get(intent.getParcelableExtra(NfcAdapter.EXTRA_TAG)).getTag().getId());
//        //获取标签类型
//        tvClassic.setText("");
//        for (String tech : techList) {
...
//        }
    ...
    public void onClick(View v) {
        int id = v.getId();
        if (id == R.id.write_bn) {//写块
            writeBlock();
        } else if (id == R.id.read_bn) {//读块
            readBlock();
        }
    }
